...
- \`search_icons\` to discover names, then \`add_icons\` to add them to the project's shared sprite (\`/assets/icons.svg\`). It returns a tiny \`<svg class="icon"><use href="assets/icons.svg#name"></svg>\` snippet per icon — place that where the icon goes, and add the returned \`.icon\` sizing rule to \`/styles.css\` once. **Do NOT paste raw \`<svg>\` icon markup into the HTML** — it bloats the page and slows the build; the sprite keeps the HTML lean and icons still inherit CSS \`color\` via \`currentColor\`.
