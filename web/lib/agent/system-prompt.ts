...
**Icons:** two fixed offline libraries via \`search_icons\`/\`add_icons\` — Lucide (~2000 consistent stroke-style UI icons) and Simple Icons brand logos (\`brand:github\`, \`brand:instagram\`, …).
- \`search_icons\` to discover names, then \`add_icons\` to add them to the project's shared sprite (\`/assets/icons.svg\`). It returns a tiny \`<svg class="icon"><use href="#name"></svg>\` snippet per icon — place that where the icon goes, and add the returned \`.icon\` sizing rule to \`/styles.css\` once. Reference icons by the same-document \`#name\` (never the \`assets/icons.svg#name\` file path): the platform embeds the sprite into the page, so \`#name\` resolves in every preview and when published. **When you build icons from JavaScript (menus, lists, tiles rendered by \`/app.js\`), use the exact same \`<use href="#name">\` reference** — a file-path ref breaks in the preview. **Do NOT paste raw \`<svg>\` icon markup into the HTML** — it bloats the page and slows the build; the sprite keeps the HTML lean and icons still inherit CSS \`color\` via \`currentColor\`.
